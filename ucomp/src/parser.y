...
prog : {push_scope();} funcvardecl progdecl {gen_code(HALT, 0); pop_scope(); YYACCEPT;}
;

funcvardecl : type TID {install($2, alloc_d());} vardecl TSCOLON funcvardecl
...
progdecl : TMAIN {install($1, gen_label()); set_main_offset(gen_label());} block
;

vardecl : TCOMMA TID {install($2, alloc_d());} vardecl
...
paramlistcont : type TID {install($2, alloc_d()); context_check(STORE, $2);}
...
block : TLBRACE {push_scope();} vardecllist commlist TRBRACE {pop_scope();}
      |	TLBRACE TRBRACE
;

vardecllist : type TID {install($2, alloc_d());} vardecl TSCOLON vardecllist
...
readvars: TCOMMA lvalexpr readvars {gen_code(READ_INT, $<litnum>2);} 
	| %empty
;

ifstmt: TIF TLPAREN expr TRPAREN {$1.addr_goto_false = alloc_c();} comm {$1.addr_goto = alloc_c(); $<lbls>$ = $1; back_patch($1.addr_goto_false, GOTO_FALSE, gen_label()); back_patch($1.addr_goto, GOTO, gen_label());}
;
...
lvalexpr : TID {$<litnum>$ = getsymbol($1, 1)->offset;}
	 | TID TLBRACK expr TRBRACK
;

primexpr : TID {context_check(LD_VAR, $1);}
